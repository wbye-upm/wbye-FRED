...
Hg          = [         9,          6,          6,      4,      5,      6,      7,         2,         3,         4,         5,         6,      5,           5] # s      - Inertia constant
...
@constraint(model, [t in 1:T], P_L[t] == Pg[1,t] / N[1])
...
@constraint(model, [t in 1:T], [y[t], 0.5, s_aux[t]] in RotatedSecondOrderCone())
# z[t] ≥ t_aux[t]^2  ⇔  (z[t], 1/2, t_aux[t])
@constraint(model, [t in 1:T], [z[t], 0.5, t_aux[t]] in RotatedSecondOrderCone())
# y[t] + z[t] ≥ x[3,t]^2  ⇔  ((y[t] + z[t])/2, 1, x[3,t])
@constraint(model, [t in 1:T], [(y[t] + z[t]) / 2, 1.0, x[3,t]] in RotatedSecondOrderCone())
